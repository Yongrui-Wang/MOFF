#!/usr/bin/env python
#!/home/ibmc-2/my_project/tools/mgltools_x86_64Linux2_1.5.7/bin pythonsh
import sys
import os
...
from gym_molecule.envs.env_utils_graph import ligsmiles, ligindiecs
import time

from xml.etree import ElementTree as ET


class covdock_gpu(object):
    """
    autodock_gpu for covalent or covalent docking
    """
    ...
    def __init__(self, docking_params):
        ...
        """
        super(covdock_gpu, self).__init__()


        self.name = docking_params['name']
        self.pdbreceptor = docking_params['receptor_pdb']
        self.rec_residue = docking_params['rec_residue']
        self.maps_file = docking_params['receptor_maps']
        self.tmp_dir = docking_params['tmp_dir']
        self.num_sub_proc = docking_params['n_process']
        ...
        self.ligsmiles = ligsmiles

        self.ligindices = ligindiecs
        # self.ligsmart = "C(=O)-O-C"
        self.residue = docking_params['rec_residue']
    ...
    def gen_align(self, smi, ligand_sdf_file, ligand_flex_file):
        """
        ...
        align_run_line = 'mk_prepare_ligand.py -i {} '.format(ligand_sdf_file)
        align_run_line += '--receptor {} '.format(self.pdbreceptor)
        align_run_line += '--rec_residue {} '.format(self.rec_residue)
        align_run_line += '--tether_smarts {} '.format('[CH-2]C')
        align_run_line += '--tether_smarts_indices {} {} '.format(1,2)
        align_run_line += '-o {} '.format(ligand_flex_file)
        # print(align_run_line)
        result = subprocess.check_output(align_run_line.split(),
                                         stderr=subprocess.STDOUT,
                                         universal_newlines=True)

        if 'RuntimeWarning' in result:

            raise RuntimeWarning(result)


    def docking(self, ligand_flex_file, ligand_xml_file):
        adgpu_run_line = '{} '.format(self.docking_program)
        adgpu_run_line += '--ffile {} '.format(self.maps_file)
        adgpu_run_line += '-F {} '.format(ligand_flex_file)
        # adgpu_run_line += '-a {} '.format(1)
        adgpu_run_line += '-x 1 '
    ...
    def docking_sub(self, q, return_dict , sub_id=0):
        ...
        while True:
            ...
            ligand_flex_file = '{}/ligand_{}_{}.pdbqt'.format(self.tmp_dir, self.name, sub_id)
            ligand_xml_file = '{}/ligand_{}_{}.xml'.format(self.tmp_dir, self.name, sub_id)



            try:
                self.gen_align(smi, ligand_sdf_file, ligand_flex_file)
            ...
            try:
                affinity_list = self.docking(ligand_flex_file, ligand_xml_file)
