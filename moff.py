...
import models.core_motif as core
from gym_molecule.envs.env_utils_graph import FRAG_VOCAB, ATOM_VOCAB, FRAG_VOCAB_MOL
...
class ReplayBuffer:
    ...
    def sample_batch(self, device, t, batch_size=32):
        ...
        n_batch = [self.n[idx] for idx in idxs]
...
def initialize_parameters(model):
    for name, param in model.named_parameters():
        if ('weight' in name) and \
                all(gnn not in name for gnn in ["gin", "gat", "sage"]):
            nn.init.xavier_normal_(param)
        elif 'bias' in name and \
                all(gnn not in name for gnn in ["gin", "gat", "sage"]):
            nn.init.zeros_(param)

def initialize_all_parameters(model):
    for name, param in model.named_parameters():
        if 'weight' in name:
            nn.init.xavier_normal_(param)
        elif 'bias' in name:
...
class sac:
    ...
    def __init__(self, writer, args, env_fn, actor_critic=core.GNNActorCritic, ac_kwargs=dict(), seed=0,
                 steps_per_epoch=4000, epochs=100, replay_size=int(1e6), gamma=0.99,
                 polyak=0.995, lr=1e-3, alpha=0.2, batch_size=100, start_steps=500,
                 update_after=100, update_every=50, update_freq=50, expert_every=5, num_test_episodes=10,
                 max_ep_len=1000,
        ...
        # self.pi_scheduler = lr_scheduler.ReduceLROnPlateau(self.pi_optimizer, factor=0.3, patience=768)
        # self.p_scheduler = lr_scheduler.ReduceLROnPlateau(self.p_optimizer, factor=0.3, patience=8, min_lr=1e-9)
        ...
        self.lipinski = args.lipinski_rew
        self.t = 0

        # self.ac.apply(xavier_uniform_init)
        self.ac.apply(xavier_normal_init)
        # self.ac.apply(initialize_parameters)
        print("p_p_num:",sum(p.numel() for p in self.p_params if p.requires_grad),"/",
              "q_p_num:",sum(p.numel() for p in self.q_params if p.requires_grad),"/",
              "emb_p_num:",sum(p.numel() for p in self.emb_params if p.requires_grad),"/",
              "pi_p_num:",sum(p.numel() for p in self.pi_params if p.requires_grad),"/")

        # _m in
        # for param in self.ac.parameters():
        #     if len(param.size()) > 1:  # 只对具有多个元素的参数进行初始化
        #         init.xavier_normal_(param)
        tm = time.localtime(time.time())
        self.init_tm = time.strftime('_%Y-%m-%d_%I.%M.%S-%p', tm)

    def compute_loss_q(self, data):
        # munchausen项需要在此采样当前的action_probs，当前只用采用ac_second 就够了

        ac_first, ac_second, ac_third, log_ac_prob = data['ac_first'], data['ac_second'], data['ac_third'], data[
            'log_ac_prob']
        ...
        with torch.no_grad():
            ...
            a2, (a2_prob, log_a2_prob), (ac2_first, ac2_second, ac2_third) = self.ac.pi(o2_g_emb, o2_n_emb, o2_g, cands,
                                                                                        data['n'])
            # Target Q-values
            q1_targ_next = self.ac_targ.q1(o2_g_emb, ac2_first, ac2_second, ac2_third)
            q2_targ_next = self.ac_targ.q2(o2_g_emb, ac2_first, ac2_second, ac2_third)
            q_pi_targ = torch.min(q1_targ_next, q2_targ_next).squeeze()
            log_ac_prob = torch.stack(log_ac_prob).squeeze()
            if bool(self.munchausen):
                # print('using munchausen')
                ### 需要更改，这里是log_ac_prob
                log_pi_a = self.m_tau * log_ac_prob.mean(1)

                munchausen_reward = r + self.m_alpha * torch.clamp(log_pi_a, min=self.lo, max=0)
                backup = munchausen_reward + self.gamma * (1 - d) * q_pi_targ  # Bellman backup
            else:
                # print('without munchausen')
        ...
        # print('Q loss', loss_q1, loss_q2, loss_q)
    ...
    def compute_loss_pi(self, data):
        ...
        _, (ac_prob, log_ac_prob), (ac_first, ac_second, ac_third) = \
            self.ac.pi(o_g_emb, o_n_emb, o_g, cands, data['n'])
        ...
        # loss_policy = torch.mean(-q_pi * sampling_score)  # sampling_score --> intr_rew
        loss_policy = torch.mean(-q_pi.squeeze() * sampling_score)
        ...
        ac_prob_comb = torch.einsum('by, bz->byz', ac_prob_sp[1], ac_prob_sp[2]).reshape(self.batch_size,
                                                                                         -1)  # (bs , 73 x 40)
        ac_prob_comb = torch.einsum('bx, bz->bxz', ac_prob_sp[0], ac_prob_comb).reshape(self.batch_size,
                                                                                        -1)  # (bs , 40 x 73 x 40)
        ...
        # loss_policy = (ac_prob_comb * (-q_pi.squeeze() * sampling_score).unsqueeze(1)).sum(dim=1).mean()
        ...
        if self.writer is not None:
            self.writer.add_scalar("Entropy", sum(-(x * ac_prob_sp[i]).mean() for i, x in enumerate(log_ac_prob_sp)),
                                   self.iter_so_far)
            self.writer.add_scalar("Alpha", alpha, self.iter_so_far)

        return loss_entropy, loss_policy, loss_alpha, q_pi
    ...
    def compute_intr_rew(self, ob, rew):
        with torch.no_grad():
            pred = self.ac.p(ob).squeeze()
            rew = rew.clone().detach().to(self.device).float()
            rew_intr = torch.abs(pred - rew).cpu().detach().numpy()
        return rew_intr

    def update(self, data, j):
        if j == 0:
            self.loss_q_list, self.loss_policy_list, \
            self.q_pi_list, self.loss_entropy_list = [], [], [], []
        ...
        # critic update
        # ==========================
        loss_q = self.compute_loss_q(data)  # loss_q --> qt - (r + qt+1)     loss_q1 + loss_q2
        ...
        self.loss_q_list.append(loss_q.item())

        # Freeze Q-networks so you don't waste computational effort
        # computing gradients for them during the policy learning step.
        for q in self.q_params:  # Freeze
            q.requires_grad = False
        # ==========================
        # policy net update
        # ==========================
        loss_entropy, loss_policy, loss_alpha, q_pi = self.compute_loss_pi(data)
        self.loss_policy_list.append(loss_policy.item())
        self.loss_entropy_list.append(loss_entropy.item())
        self.q_pi_list.append(q_pi.mean().item())
        ...
        if (j+1) % 256 == 0:
            print(j+1)
            print("loss_q_mean {}".format(round(sum(self.loss_q_list)/len(self.loss_q_list), 4)),",",
                  "loss_policy {}".format(round(sum(self.loss_policy_list)/len(self.loss_policy_list), 4)),",",
                  "loss_entropy {}".format(round(sum(self.loss_entropy_list)/len(self.loss_entropy_list), 4)),",",
                  "q_pi {}".format(round(sum(self.q_pi_list)/len(self.q_pi_list), 4)))
            print("grad_pi {:.4g}".format(np.array(ave_pi_grads).mean()),",",
                  "grad_q {:.4g}".format(np.array(ave_q_grads).mean()))
            print("pi_lr {:.3g}".format(self.pi_optimizer.param_groups[-1]["lr"]))
            print("q_lr {:.3g}".format(self.q_optimizer.param_groups[-1]["lr"]))
    ...
    def train(self):
        ...
        for t in range(total_steps):
            ...
            with torch.no_grad():
                ...
                if t > self.start_steps:
                    ac, (ac_prob, log_ac_prob), (ac_first, ac_second, ac_third) = \
                        self.ac.pi(o_g_emb, o_n_emb, o_g, cands, self.env.n)
                    # print(ac, ' pi')
                else:
                    ac = self.env.sample_motif(self.env.n, self.step_list)[np.newaxis]
                    (ac_prob, log_ac_prob), (ac_first, ac_second, ac_third) = \
                        self.ac.pi.sample(ac[0], o_g_emb, o_n_emb, o_g, cands, self.env.n)
                    # print(ac, 'sample')
            a2 = time.time()
            # Step the env
            ...
            if any(o2['att']):
                ...
                if type(ac) == np.ndarray:
                    self.replay_buffer.store(o, ac, r, o2, r_d,
                                             ac_prob, log_ac_prob, ac_first, ac_second, ac_third,
                                             o_embeds, intr_rew, self.env.n - 1)
                else:
            ...
            if t > 1 and t % self.docking_every == 0 and self.env.smile_list != []:
                ...
                if n_smi > 0:
                    if self.lipinski:
                        DS = self.env.reward_batch()
                        QED, SA = self.env.SAandQEDscores()
                        if t <= self.update_after:
                            DSs.extend(DS)
                            SAs.extend(SA)
                        else:
                            print("DSmean, DSstd:", np.array(DSs).mean(), np.array(DSs).std())
                            print("SAmean, SAstd:", np.array(SAs).mean(), np.array(SAs).std())
                        # 记录为都记录，但是奖励函数只使用结果
                        normalized_DS = (DS-np.array(DSs).mean())/np.array(DSs).std()
                        normalized_SA = (SA-np.array(SAs).mean())/np.array(SAs).std()
                        ext_rew = (normalized_DS * QED * -normalized_SA) * 10
                    else:
                        ext_rew = self.env.reward_batch()

                    if self.active_learning == "freed_pe" or 'kingrose':
                        ave_p_grads=[]
                        ...
                        if not torch.isnan(loss_p).any():  # reward_prediction迭代
                            ...
                            for p in self.p_params:
                                    ave_p_grads.append(p.grad.abs().mean().item())
                            ...
                            # print("loss_p: {}".format(round(loss_p.item(), 4)),",",
                            #       "p_lr {:.3g}".format(self.p_optimizer.param_groups[-1]["lr"]),',',
                            #       "p_grad {:.4g}".format(np.array(ave_p_grads).mean()))
                        for e in self.emb_params:
                    ...
                    with open(self.fname[:-3] + self.init_tm + '.csv', 'a') as f:
                        for i in range(n_smi):
                            if self.lipinski:
                                str = f'{self.env.smile_list[i]},{DS[i]},{np.round(QED[i], 3)},{np.round(SA[i], 3)},{np.round(ext_rew[i], 3)},{t}' + '\n'
                            else:
            ...
            if t >= self.update_after and t % self.update_every == 0:
                if t > self.start_steps:
                    print("==========================================")
                    os.system("python3 molecule_gen/enrichment_running.py "
                              +self.fname[:-3]+self.init_tm+'.csv')
                    print("==========================================")
                ...
                for j in tqdm(range(self.update_every)):
                    ...
                    if (j+1) % 257 == 0:
                        print('update2 time: {}'.format(round((time.time() - s_update), 4)))
                        print("------------------------------------------")
                dt_update = time.time()
                print('total update time: {}'.format(round((time.time() - t_update), 4)))
                print("alpha: {}".format(round(self.log_alpha.exp().item(), 5)))
